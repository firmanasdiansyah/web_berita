	...
	<?php
		...
		if(isset($_POST['submit'])){
            ...
            $sql = "UPDATE posts SET 
					...
					title='$ftitle',
					content='$fcontent',
					position_id='$fposition',
					date='$fdate',
					WHERE id_posts='$id'";

            $datap = $conn->query($sql);

            if (mysqli_affected_rows($conn)>0){
                header("Location:index.php");
            }else {
                $_SESSION['error']= "Mengedit data gagal!";
            }
        }
	?>
	<!-- Navbar -->
			...
			<main class="col-md-9 ms-sm-auto col-lg-10 px-md-4 py-4 " >
				<nav aria-label="breadcrumb">
					<ul class="breadcrumb">
						...
						<li class="breadcrumb-item ">
							<a class="link-website2"  href="#">Edit Posts</a>
						</li>
					</ul>
				</nav>
				<h1>Edit Posts</h1>
				...
				<div class="card">
					...
					<form action="<?php echo $_SERVER['PHP_SELF']."?id=".$id ?>" method="post" >
							<div class="mb-3">
								<label for="fauthor" class="form-label"> Authors </label>
								<select class="form-select" aria-label="Default select example" name="fauthor">
									<option value="<?php echo $author_id ?>"> <?php echo $author ?>  </option>
									<?php foreach($authors as $dauthor): ?>
										<option value="<?php echo ($dauthor['id']) ?>">
										<?php echo ($dauthor['name']) ?></option>
									<?php endforeach; ?>
								</select>
                            </div>
							<div class="mb-3">
								...
								<select class="form-select" aria-label="Default select example" name="fcategory">
									...
									<?php foreach($categories as $dcategory): ?>
										<option value="<?php echo ($dcategory['id']) ?>">
										<?php echo ($dcategory['category']) ?></option>
									<?php endforeach; ?>
								</select>
                            </div>
							<div class="mb-3">
								<label for="ftitle" class="form-label"> Title </label>
								<input type="text" class="form-control" id="ftitle" name="ftitle"  require value="<?php echo $title ?>" >
							</div>
							<div class="mb-3">
								<label for="fcontent" class="form-label"> Content </label>
								<textarea class="form-control" id="fcontent"  require name="fcontent" value="<?php echo $content ?>" ><?php echo $content ?></textarea>
							</div>
							<div class="mb-3">
								<label for="fdate" class="form-label"> Date </label>
								<input type="date" class="form-control" id="fdate" name="fdate"  require value="<?php echo $date ?>" >
							</div>
							<div class="mb-3">
								<label for="fdate" class="form-label"> Picture </label> <br>
								<img src="img/<?php echo $picture ?>" width="150" height="100"><br><?php echo $picture ?>
							</div> 
							<div class="mb-3">
								<label for="fposition" class="form-label"> Position </label>
								<select class="form-select" aria-label="Default select example"  id="fposition" name="fposition">
									<option value="<?php echo $position_id ?>"><?php echo $position ?>  </option>
									<?php foreach($positions as $dposition): ?>
										<option value="<?php echo ($dposition['id']) ?>">
										<?php echo ($dposition['position']) ?></option>
									<?php endforeach; ?>
								</select>
                            </div>
