...
<body>
	<?php
		...
		$sql = "SELECT * FROM posts 
                ...
                WHERE position='index' ORDER BY posts.date DESC;";

        if(isset($_POST['filter'])){
			$tgl = $_POST['date'];
			$category = $_POST['category'];
			if($_POST['date'] !== "" and $_POST['category'] !== ""){
				$sql = "SELECT * FROM posts 
				JOIN authors ON posts.author_id = authors.id 
				JOIN categories ON posts.category_id = categories.id
				JOIN positions ON posts.position_id = positions.id
				WHERE position='index' and posts.category_id = '$category' and posts.date = '$tgl'";
			}elseif($_POST['category'] !== ""){
				$sql = "SELECT * FROM posts 
				JOIN authors ON posts.author_id = authors.id 
				JOIN categories ON posts.category_id = categories.id
				JOIN positions ON posts.position_id = positions.id
				WHERE position='index' and posts.category_id = '$category'";
			}elseif($_POST['date'] !== ""){
				$sql = "SELECT * FROM posts 
				JOIN authors ON posts.author_id = authors.id 
				JOIN categories ON posts.category_id = categories.id
				JOIN positions ON posts.position_id = positions.id
				WHERE position='index' and posts.date = '$tgl'";
			}
		}
		  ...
		  <div class="container mb-2">
		    ...
		    <div class="collapse navbar-collapse" id="navbarSupportedContent">
				...
				<div class="col-2">
					<ul class="navbar-nav ">
						<li class="nav-item">
							<a class="nav-link " aria-current="page" href="/web_berita/">Home</a>
						</li>
                        <li class="nav-item">
							<a class="nav-link active" aria-current="page" href="/web_berita/categories.php">Categories</a>
						</li>
						<li class="nav-item">
	...
	<div class="container mt-4 mb-3">
        <form action="<?php echo $_SERVER['PHP_SELF']; ?>" method="POST">
            <div class="row">
                ...
                <div class="col-2">
                    <button type="submit" class="btn btn-dark " name="filter"> Search </button>
                </div>
            </div>
        </form >
	</div>
	...
	<div class="container mt-3">
		...
		<div class="row ">
			<div class="col-md-3">
				<img src="admin/posts/img/<?php echo $data['picture'] ?>" width="270" height="200" class="d-inline-block align-top" alt="">
			</div>
			<div class="col-md-8">
				<div class="card-body">
					<a class="link-website" href="detail.php?id=<?php echo$data['id_posts']?>"><h3> <?php echo $data['title']; ?></h3></a>
